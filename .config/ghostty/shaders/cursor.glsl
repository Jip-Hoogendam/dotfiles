//shader is taken from https://github.com/sahaj-b/ghostty-cursor-shaders

// -- CONFIGURATION --
vec4 TRAIL_COLOR = iCurrentCursorColor; // can change to eg: vec4(0.2, 0.6, 1.0, 0.5);
const float TRAIL_TRANSPARENCY = 0.7;
const float DURATION = 0.09; // in seconds
const float MAX_TRAIL_LENGTH = 0.2;
const float THRESHOLD_MIN_DISTANCE = .1; // min distance to show trail (units of cursor width)
const float BLUR = 0.002; // blur size in pixels (for antialiasing)
...
float antialising(float distance) {
  return clamp(0.5 - distance / BLUR, 0.0, 1.0);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (lineLength > minDist && progress < 0.9999999999999) {
    // ANIMATION logic
